<template>
  <v-layout class="rounded rounded-md main ">
    <v-app-bar title="4DayForecast"></v-app-bar>
...
interface WeatherData {
  ...
  weatherFutureData: WeatherFutureData;
}
...
const expands = ref<boolean>(false);
const searchInput = ref<string>('');
const weatherTUrl = ref<string>('');
let weatherTodayData = reactive<WeatherTodayData>({
  adcode: '',
  city: '',
  humidity: '',
  province: '',
...
let weatherFutureData = reactive<WeatherFutureData>({
  adcode: '',
  casts: [{
    date: '',
    daypower: '',
...
// 删除标签卡
const deleteTab = (adcode: string): void => {
...
// Axios获取天气数据
const fetchWeatherTData = (cityCode: string): void => {
...
const fetchWeatherFData = (cityCode: string): void => {
  axios.get('https://restapi.amap.com/v3/weather/weatherInfo', {
    params: {
      key: apikey,
      city: cityCode,
